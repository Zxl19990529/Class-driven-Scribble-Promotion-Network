...
class ResNet_CAMATT(nn.Module):
    def __init__(self, block, layers, num_classes,BatchNorm=nn.BatchNorm2d):
        ...
        if block.__name__ == 'Bottleneck':
            self.layer_pred0 = self._make_pred_layer(ASPP, 2048, [6,12,18,24], BatchNorm, num_classes) # train with scribble
            self.layer_camatt = CAMMATT(in_dims=2048,out_dims=2048)
        else:
            self.layer_pred0 = self._make_pred_layer(ASPP, 512, [6,12,18,24], BatchNorm, num_classes)
            self.layer_camatt = CAMMATT(in_dims=512,out_dims=512)
    ...
    def forward(self, x,scribble,cam): # scribble: b,h,w 0,1,2..  cam: b,h,w 0,1,2...
        ...
        x3 = self.layer3(x2)
        x4 = self.layer4(x3)
        out0 = self.layer_pred0(x4)
        #
        att_loss = self.layer_camatt(x4,cam)
        return out0,att_loss
    def forward_eval(self,x):
        x = self.conv1(x)
        x = self.bn1(x)
        x = self.relu(x)
        x = self.maxpool(x) # 1,64,117,117
        x1 = self.layer1(x) # 1,256,117,117
        x2 = self.layer2(x1) # 1,512,59,59
        x3 = self.layer3(x2) # 1,1024,59,59
        x4 = self.layer4(x3) # 1,2048,59,59
        out0 = self.layer_pred0(x4)
        return out0

class CAMMATT(nn.Module):
    def __init__(self,in_dims=2048,out_dims=2048):
        super(CAMMATT, self).__init__()
    ...
    # def forward(self,x,cam,mode='cam'): #  cam: b,h,w 0,1,2... 
    #     cam[cam == 255] = 0
    ...
    # def forward(self,x,cam,mode='cam'): #  cam: b,h,w 0,1,2... # CMATT_baseline_xplusnewx
    #     cam[cam == 255] = 0
    #     new_x = torch.zeros_like(x) # b,c,h,w
    #     x_shape = x.size()[2:4] # x: b,c,h,w
    #     B,C,H,W = x.size()
    ...
    #     atten = torch.softmax(atten,dim=-1) # b,hw(query),hw(key,softmaxed)
    #     atten = curr_mask * atten # b,hw(query,masked),hw(key,softmaxed)
    #     x_tmp = x.view(B,-1,H*W).permute(0,2,1) # b,c,h,w -> b,c,hw -> b,hw,c   here I forgot to add curr_mask
    #     new_x = x + torch.bmm(atten,x_tmp).permute(0,2,1).view(B,C,H,W)# b,hw(query,masked),c(weighted channel) -> b,c,hw -> b,c,h,w # gamma=0
    #     new_x = self.gamma * new_x
    #     loss = self.distance_func(x,new_x)
    #     return loss 
    
    # def forward(self,x,cam,mode='cam'): #  cam: b,h,w 0,1,2... # CMATT_baseline_gamafirst
    #     cam[cam == 255] = 0
    #     new_x = torch.zeros_like(x) # b,c,h,w
    #     x_shape = x.size()[2:4] # x: b,c,h,w
    #     B,C,H,W = x.size()
    ...
    #     atten = curr_mask * atten # b,hw(query,masked),hw(key,softmaxed)
    #     x_tmp = x.view(B,-1,H*W).permute(0,2,1) # b,c,h,w -> b,c,hw -> b,hw,c   here I forgot to add curr_mask
    #     new_x = x + self.gamma *torch.bmm(atten,x_tmp).permute(0,2,1).view(B,C,H,W)# b,hw(query,masked),c(weighted channel) -> b,c,hw -> b,c,h,w gamma = 1
    #     loss = self.distance_func(x,new_x)
    #     return loss 
