...
parser.add_argument('--model_type',default='deeplabv3p',help='Model type selection. nonRW|RW|res50_cam|res50_labelFusion|res50_CAMATT|res50_deeplabv3_camatt|deeplabv3p')
...
parser.add_argument('--l_seg',default=1,type=float,help='The loss weight of the segmentation loss')
parser.add_argument('--l_pesudo',default=0,type=float,help='The loss weight of the pesudo labels')
parser.add_argument('--l_CAMATT',default=0,type=float,help='The loss of cam attention loss, must used with res50_CAMATT')
parser.add_argument('--label_smooth',default=0,type=float,help='label smooth for pesudo labels generated by CAM methods. 0~0.9 use CE loss with label smoothing; -1 use the distance map to smooth the pesudo labels')
parser.add_argument('--distanceScrib_entropy',default=0,type=float,help='Use distanceminentropy loss of scribble basing on distance map.')
parser.add_argument('--distanceCAM_entropy',default=0,type=float,help='Use distanceminentropy loss of CAM basing on distance map.')
parser.add_argument('--labelFusion_entropy',default=0,type=float,help='Use the label fusion module to do the entropy minimium loss')
# dataset
parser.add_argument('--dataset_path', metavar='DATASET_PATH',default='dataset/SBD_leige', help='path to the dataset(multiple paths are concated by "+")')
...
parser.add_argument('--distance_map_Scrib',default='distance_map_lambd_1',help='The path of the scribble distance map pngs')
parser.add_argument('--distance_map_CAM',default='distance_CAM_lambd_50',help='The path of the CAM distance map pngs')
# train paramters
...
class Trainer():
    def __init__(self, args):
        ...
        elif args.label_smooth == -1:
            self.pesudolabel_entropy = loss.DistanceMapLoss(numclass=args.numclasses)

        if args.distanceScrib_entropy >0 or args.distanceCAM_entropy>0:
            self.distanceMap_entropy_loss = loss.DistanceMapMinEntropy(numclass=args.numclasses)
        else:
            self.distanceMap_entropy_loss = None
        
        if args.labelFusion_entropy >0:
            self.labelFusion_entropy_loss = loss.DistanceMinEntropy(numclass=args.numclasses)
        else:
            self.labelFusion_entropy_loss = None
    ...
    def train(self, epoch):
        ...
        for i, batch in enumerate(tbar):
            ...
            if args.dataset == 'VOC2012':
                ...
                img_v = img.to(device)
                gt_v = gt.to(device)
                edge_v =edge.to(device)
                batch_size = img.size()[0]
                input_size = img.size()[2:4]
            elif args.dataset == 'ScribbleOnly': # only scribbles are used
                img, gt, path_name = batch
                img_v = img.to(device)
                gt_v = gt.to(device)
                batch_size = img.size()[0]
                input_size = img.size()[2:4]
            elif args.dataset == 'SBD_pesudo': # 
                img,gt,label_pesudo,path_name = batch
                img_v = img.to(device)
                gt_v = gt.to(device)
                ...
                img,gt,label_pesudo,distance_scrib,distance_CAM,path_name = batch
                img_v = img.to(device,non_blocking=True)
                gt_v = gt.to(device,non_blocking=True)
                label_pesudo_v = label_pesudo.to(device,non_blocking=True) # b,h,w
                distance_scrib_v = distance_scrib.to(device,non_blocking=True) # b,h,w
                distance_CAM_v = distance_CAM.to(device,non_blocking=True) # b,h,w
                batch_size = img.size()[0]
                input_size = img.size()[2:4]
            elif args.dataset == 'scribble_pseudo_dsmp':
                img,gt,label_pesudo,distance_scrib,path_name = batch
                img_v = img.to(device,non_blocking=True)
                gt_v = gt.to(device,non_blocking=True)
                label_pesudo_v = label_pesudo.to(device,non_blocking=True) # b,h,w
                distance_scrib_v = distance_scrib.to(device,non_blocking=True) # b,h,w
            ...
            with amp.autocast():
                if args.model_type == 'res50_CAMATT':
                    pred_seg,camatt_loss = self.model(img_v,gt_v,label_pesudo_v)
                ...
                if torch.min(gt_v[gt_v.ne(255)]) <0 or torch.max(gt_v[gt_v.ne(255)]) >=args.numclasses:
                    print('gt_v',path_name,torch.unique(gt_v[gt_v.ne(255)]))
                    
                loss_seg = self.criterion_CE(pred_sg_up, gt_v.squeeze(1))
                loss = args.l_seg * loss_seg
                ...
                if args.label_smooth >0:
                    ...
                    loss = loss + args.l_pesudo* loss_pesudolabel
                    ...
                    loss_pesudolabel = self.pesudolabel_entropy(pred_sg_up,label_pesudo_v,distance_scrib_v)
                    if local_rank == 0:
                        writer.add_scalar('train/pesudolabel_loss',float(loss_pesudolabel),epoch*len(tbar)+i)
                    loss = loss + args.l_pesudo*loss_pesudolabel
                    
                if args.distanceScrib_entropy > 0:
                    loss_distanceScrib_entropy =  self.distanceMap_entropy_loss(pred_sg_up,distance_scrib_v)
                    if local_rank == 0:
                        writer.add_scalar('train/distanceScrib_entropy',float(loss_distanceScrib_entropy),epoch*len(tbar)+i)
                    loss = loss + args.distanceScrib_entropy * loss_distanceScrib_entropy
                if args.distanceCAM_entropy > 0:
                    loss_distanceCAM_entropy =  self.distanceMap_entropy_loss(pred_sg_up,distance_CAM_v)
                    if local_rank == 0:
                        writer.add_scalar('train/distanceCAM_entropy',float(loss_distanceCAM_entropy),epoch*len(tbar)+i)
                    loss = loss + args.distanceCAM_entropy * loss_distanceCAM_entropy
                if args.l_CAMATT > 0:
                    camatt_loss = args.l_CAMATT*camatt_loss
                    if local_rank == 0:
                        writer.add_scalar('train/labelFusino_entropy',float(camatt_loss),epoch*len(tbar)+i)
                    loss = loss + camatt_loss
    ...
    def validate_tnt(self, epoch):
        ...
        with torch.no_grad():
            for i, batch in enumerate(tbar):
                ...
                # with amp.autocast():
                if args.model_type == 'res50_labelFusion':
                    pred_seg = self.model.forward_eval(img_v)
                elif args.model_type == 'res50_CAMATT':
                    pred_seg = self.model.forward_eval(img_v)
                elif args.model_type == 'deeplabv3p':
                    pred_seg = self.model.forward_eval(img_v)
                elif args.model_type == 'deeplabv2':
                    pred_seg = self.model.forward(img_v)
                elif args.model_type == 'res50_ASPP_CAMATT_bak':
                    pred_seg = self.model.forward_eval(img_v)
                elif args.model_type == 'res50_ASPP_CAMATT_pure':
                    pred_seg = self.model.forward_eval(img_v)
                elif args.model_type == 'res50_deeplabv3_camatt':
                    pred_seg = self.model.forward_eval(img_v)
                else:
                    pred_seg,pred_class= self.model.forward(img_v)
                if args.distributed:
                    batch_img = [torch.zeros_like(img_v) for _ in range(world_size)]
                    batch_gt = [torch.zeros_like(gt_v) for _ in range(world_size)]
                    batch_pred_seg = [torch.zeros_like(pred_seg) for _ in range(world_size)]
                    dist.all_gather(batch_img,img_v)
                    dist.all_gather(batch_gt,gt_v)
                    dist.all_gather(batch_pred_seg,pred_seg)
                    img_v = torch.cat(batch_img,dim=0)
                    gt_v = torch.cat(batch_gt,dim=0)
                ...
                if i == 0 and local_rank == 0:
                    ...
                    label=cp[gt_v.cpu(),:].squeeze().permute(0,3,1,2)
                    ...
                    writer.add_image('gt&pred_seg',img_grid,epoch)
